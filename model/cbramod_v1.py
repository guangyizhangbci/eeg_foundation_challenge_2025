...
class CBraMod(nn.Module):
    def __init__(self, in_dim=100, out_dim=100, d_model=100, dim_feedforward=400,
                 seq_len=8, n_layer=12, nhead=4, param=None):
        super().__init__()
    ...
    def forward(self, x, mask=None):
        ...
        if not hasattr(self, 'hann_win') or self.hann_win.shape[-1] != x.shape[-1]:
            self.register_buffer('hann_win',
                                 ...
                                 persistent=False)
        x = torch.nn.functional.layer_norm(x * self.hann_win, x.shape[-1:])
        ...
        patch_emb = patch_emb.permute(0, 2, 1, 3).contiguous().view(B, C, P, -1)
