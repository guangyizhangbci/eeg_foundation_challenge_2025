...
def get_data_loader():
    ...
    all_train_y = torch.cat([torch.as_tensor(ds.y, dtype=torch.float32) for ds in train_set.datasets])
    y_mean, y_std = all_train_y.mean(), all_train_y.std()

    loader_kwargs = dict(batch_size=512, pin_memory=True, num_workers=12)
    return {
        "train": DataLoader(train_set, shuffle=True,  **loader_kwargs),
        "val":   DataLoader(valid_set, shuffle=False, **loader_kwargs),
